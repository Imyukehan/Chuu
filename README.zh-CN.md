...
3. 运行 `xcodegen generate --spec project.yml`，打开 `Chuu.xcodeproj`，选择 `Chuu` scheme，运行。
...
作者目前只有几只鼠标，因此适配范围还很有限。欢迎为新的厂商和型号提交适配！目前实测为 Logitech G502 X PLUS（2.4 GHz 接收器 `046D:C547`）和 MCHOSE G7（`A8A5:2255`）；同品牌、相似名称或同系列接收器不代表协议兼容。

适配代码按 `Chuu/Devices/<厂商>/` 组织，统一在 `MouseAdapterRegistry` 注册。请参考[鼠标适配指南](docs/mouse-adapters.md)、[可编译的模拟示例](Examples/MouseAdapter/ExampleMouseAdapter.swift)和[贡献指南](CONTRIBUTING.md)，提交型号、固件、连接方式、接口信息、去除隐私的响应样本及真实设备验证结果。

目录和兼容性说明见[项目结构](docs/architecture.md)。源码、测试、小组件分别位于 `Chuu/`、`ChuuTests/`、`ChuuWidget/`，只保留一个 `Chuu.xcodeproj`。
...
Chuu 是独立的社区项目，不是任何鼠标厂商的官方应用。品牌和型号名称仅用于识别设备；适配某款设备不代表与其厂商存在合作或获得认可。

设备外观可能使用不同来源的图片、渲染预览或模型。第三方素材及商标的权利归各自权利人所有，不随 Chuu 的源码许可一并授权。具体来源与使用说明统一记录在 [NOTICES](NOTICES.md)，包括仅用于本机原型的可选素材。通过其他开源工具获取素材，不代表素材适用该工具的代码许可。

硬件适配仍在开发中，仅以文档列出的型号和连接方式为准。修改板载设置前请保留备份；未经测试的设备或固件版本可能表现不同。
