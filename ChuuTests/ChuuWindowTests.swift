...
final class ChuuWindowTests: XCTestCase {
    @MainActor
    func testMainWindowHasFixedLandscapeSize() throws {
        let window = try XCTUnwrap(ChuuWindow.shared.window)
    ...
    func testClosingMainWindowHidesDockEvenWithTransientPanel() {
        ...
        ChuuWindow.shared.windowWillClose(Notification(name: NSWindow.willCloseNotification))
