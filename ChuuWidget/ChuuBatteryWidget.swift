...
struct MouseBatteryView: View {
        ...
        Group {
            if devices.isEmpty {
                VStack(spacing: 12) {
                    ...
                    Text(NSLocalizedString("No mouse connected", tableName: "Chuu", comment: "Widget empty state")).font(.caption)
    ...
    private func tile(_ device: MouseSnapshot) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 7) {
            ...
            HStack(spacing: 4) {
                Text(device.battery == nil ? NSLocalizedString("Battery unavailable", tableName: "Chuu", comment: "Widget state") :
                    fresh && device.online ? device.connectionLabel : NSLocalizedString("Last seen", tableName: "Chuu", comment: "Widget stale state"))
...
@main
struct ChuuBatteryWidget: Widget {
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: MouseSnapshotStore.widgetKind, provider: BatteryProvider()) { MouseBatteryView(entry: $0) }
            ...
            .description(NSLocalizedString("Mouse battery", tableName: "Chuu", comment: "Widget gallery description"))
