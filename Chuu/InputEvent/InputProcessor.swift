...
class InputProcessor: ModifierFlagsProviding {
    ...
    private struct ActiveBindingSession {
        ...
        var mosScrollTapReplay: ChuuScrollTapReplay?
    }

    private struct ChuuScrollTapReplay {
    ...
    private func mosScrollTapReplay(for action: ResolvedAction, event: InputEvent) -> ChuuScrollTapReplay? {
        guard case .mosScroll = action,
              let context = ShortcutExecutor.shared.mouseTapReplayContext(for: event) else {
            return nil
        }
        return ChuuScrollTapReplay(
            context: context,
    ...
    private func shouldReplayMosScrollTap(_ tapReplay: ChuuScrollTapReplay, releaseEvent: InputEvent) -> Bool {
