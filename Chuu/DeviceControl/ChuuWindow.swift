...
final class ChuuNavigation: ObservableObject {
    enum Page: String, CaseIterable {
        ...
        var title: String {
            ...
            return NSLocalizedString(key, tableName: "Chuu", comment: "Navigation tab")
        }
    }
...
@available(macOS 14.0, *)
final class ChuuWindow: NSWindowController, NSWindowDelegate, NSToolbarDelegate {
    static let shared = ChuuWindow()
    private static let navigationID = NSToolbarItem.Identifier("Chuu.Navigation")
    private let navigation: ChuuNavigation
    ...
    private init() {
        ...
        let hostingController = NSHostingController(rootView: ChuuView(model: .shared, navigation: navigation))
    ...
    func toolbar(_ toolbar: NSToolbar, itemForItemIdentifier itemIdentifier: NSToolbarItem.Identifier,
        ...
        let item = NSToolbarItemGroup(itemIdentifier: itemIdentifier,
                                      titles: ChuuNavigation.Page.allCases.map(\.title),
                                      selectionMode: .selectOne, labels: nil,
                                      target: self, action: #selector(selectPage(_:)))
        item.controlRepresentation = .expanded
        item.selectedIndex = ChuuNavigation.Page.allCases.firstIndex(of: navigation.page) ?? 0
...
@available(macOS 14.0, *)
struct ChuuPreferencesPane: NSViewControllerRepresentable {
